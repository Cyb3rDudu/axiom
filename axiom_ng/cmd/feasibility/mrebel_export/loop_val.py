...
wp=ort.InferenceSession("/models/mrebel_onnx/decoder_with_past_model.onnx",providers=_op(dev))
...
with torch.no_grad():
    ref=m.model.decoder(input_ids=ids2,encoder_hidden_states=torch.from_numpy(eh).to(dev),encoder_attention_mask=torch.from_numpy(em).to(dev))
