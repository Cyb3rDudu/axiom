...
func main() {
	...
	if err != nil { fatal("read: %v", err) }
	var chunks []chunk
	if err := json.Unmarshal(raw, &chunks); err != nil { fatal("json: %v", err) }
	...
	encodeOne := func(text string) ([]float32, error) {
		...
		inIDs, er := ort.NewTensor(shape, ids64); if er != nil { return nil, er }
		defer inIDs.Destroy()
		mask := make([]int64, n)
		for i := range mask { mask[i] = 1 }
		inMask, er := ort.NewTensor(shape, mask); if er != nil { return nil, er }
		defer inMask.Destroy()
		outSent, er := ort.NewEmptyTensor[float32](ort.NewShape(1, 1024)); if er != nil { return nil, er }
		defer outSent.Destroy()
		outTok, er := ort.NewEmptyTensor[float32](ort.NewShape(1, n, 1024)); if er != nil { return nil, er }
		defer outTok.Destroy()
		if er := sess.Run([]ort.Value{inIDs, inMask}, []ort.Value{outTok, outSent}); er != nil { return nil, er }
	...
	for i := range run1 { if i >= len(run2) || math.Float32bits(run1[i]) != math.Float32bits(run2[i]) { det = false; break } }
	fmt.Printf("DETERMINISM (coreml chunk0 run1 vs run2): %v\n", det)
	var s float64
	for _, x := range run1 { s += float64(x) * float64(x) }
	...
	for ci := 0; ci < 3; ci++ {
		...
		var tmp [4]byte
		for _, x := range emb { binary.BigEndian.PutUint32(tmp[:], math.Float32bits(x)); out.Write(tmp[:]) }
		out.Close()
	}
}
