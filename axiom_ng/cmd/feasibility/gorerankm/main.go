...
func main() {
	...
	fmt.Printf("RERANK-75 LATENCY total=%s avg=%s\n", elapsed, elapsed/75)
	if !det { os.Exit(3) }
...
func sessionOpts() *ort.SessionOptions {
	...
	if err := opts.AppendExecutionProviderCoreMLV2(map[string]string{}); err != nil { fatal("coreml ep: %v", err) }
	fmt.Fprintln(os.Stderr, "[gorerankm] using CoreML EP (Mac)")
	return opts
}
