...
func main() {
	...
	if err != nil { fatal("read meta: %v", err) }
	var m metaT
	if err := json.Unmarshal(jraw, &m); err != nil { fatal("meta: %v", err) }
	raw, err := os.ReadFile(binPath)
	if err != nil { fatal("read bin: %v", err) }
	...
	if os.Getenv("ORT_CUDA") == "1" {
		...
		if err != nil { fatal("cudaopts: %v", err) }
		dev := os.Getenv("ORT_CUDA_DEVICE"); if dev == "" { dev = "0" }
	...
	for _, name := range m.Names {
		...
		if m.IsBool[name] {
			bb := make([]bool, n)
			for i := 0; i < n; i++ { bb[i] = raw[off+i] != 0 }
			t, er := ort.NewTensor(ort.NewShape(i64s(shape)...), bb)
			if er != nil { fatal("bool tensor %s: %v", name, er) }
			inputs = append(inputs, t)
			defer t.Destroy()
		} else {
			ids := make([]int64, n)
			for i := 0; i < n; i++ { ids[i] = int64(int64fromLE(raw[off+i*8:])) }
			t, er := ort.NewTensor(ort.NewShape(i64s(shape)...), ids)
			if er != nil { fatal("i64 tensor %s: %v", name, er) }
			inputs = append(inputs, t)
			defer t.Destroy()
		}
	}
	...
	if er != nil { fatal("out: %v", er) }
	defer out.Destroy()
	session, er := ort.NewAdvancedSession(model, m.Names, []string{"logits"}, inputs, []ort.Value{out}, opts)
	if er != nil { fatal("session: %v", er) }
	defer session.Destroy()
	if er := session.Run(); er != nil { fatal("run: %v", er) }

	d := out.GetData()
	f, _ := os.Create(outBin)
	defer f.Close()
	var tmp [4]byte
	for _, v := range d { binary.BigEndian.PutUint32(tmp[:], math.Float32bits(v)); f.Write(tmp[:]) }
...
func i64s(s []int) []int64 {
	out := make([]int64, len(s)); for i, x := range s { out[i] = int64(x) }; return out
}
func total(s []int) int {
	n := 1; for _, x := range s { n *= x }; return n
}
