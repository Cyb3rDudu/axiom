...
with --device cuda and --no-fp16 (the historical fp32 behavior; the old
positional device arg, if given, still works).
...
cmd = [sys.executable, os.path.join(here, "pydense_ref.py"),
       *sys.argv[1:3], sys.argv[3] if len(sys.argv) > 3 else "/tmp",
       "--device", sys.argv[4] if len(sys.argv) > 4 else "cuda", "--no-fp16"]
