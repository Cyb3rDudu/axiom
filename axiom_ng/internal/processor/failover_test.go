...
func TestFailover_SubmitFallsBackWhenPrimaryDown(t *testing.T) {
	...
	// Routing is forgotten after ack (bounded memory).
	fc.mu.Lock()
...
func TestFailover_RouteIsolation(t *testing.T) {
	...
	// Status polls: primary answers only for its own job — distinguish by
	// hit counts on /v1/process already done; here route via ack behavior.
