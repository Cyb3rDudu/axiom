...
// order assigns to Gate 4; until then axiom-ng cannot truthfully acknowledge a
// result, so a job whose result cannot be persisted is failed, never completed
// and never acknowledged. ACK (persisted:true) is sent ONLY after a persister
// returns a snapshot id, so a processor is never told its result was committed
// when nothing durable exists (Gate 2 finding F1).
type ResultPersister interface {
	// PersistResult durably commits result for jobID and returns the snapshot id.
	PersistResult(ctx context.Context, jobID string, result []byte) (string, error)
}

// errPersister is the production stance until Gate 4: no real persistence yet,
// so any job reaching completion must fail rather than be marked completed with
// no durable snapshot. Tests inject a recording fake persister instead.
type errPersister struct {
	msg string
}

func (e *errPersister) PersistResult(context.Context, string, []byte) (string, error) {
	return "", e
}
