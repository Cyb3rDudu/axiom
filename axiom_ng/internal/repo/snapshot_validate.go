...
func validateLocatorsAndRelationships(res *processor.Result, frozen *FrozenInput) error {
	...
	for _, c := range res.Chunks {
		...
		case "page_span":
			...
			case processor.PageSourceFolioVerified, processor.PageSourcePDFLabelSane, processor.PageSourcePhysicalOnly, processor.PageSourceBlind, processor.PageSourceFolioInterpolated:
			case "":
