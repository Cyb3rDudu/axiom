...
// (dispatcher instances) plus 26000 (the new production API port). Opt out
// for local dev with AXIOM_ALLOW_DEBUG_BIND=1.
func DebugBindRefused(buildType string, port int, allowEnv func(string) string) bool {
	if buildType == BuildTypeRelease || allowEnv("AXIOM_ALLOW_DEBUG_BIND") == "1" {
		return false
	}
	return (port >= 8011 && port <= 8015) || port == 26000
}
