...
Definitionen: P@5 = Gold-Buch-Treffer / Top-5; MRR = 1/Rang des ersten Gold-Treffers; R@10 = gefundene Gold-Bücher / |Gold|. top_n=10, Overfetch 3×=30 Rerank-Kandidaten.
...
3. **Sparse-Arm (R5): kein Gewinn, hohe Kosten.** MRR −0.027 vs. hybrid+rerank, R@10 −0.020; dafür ~+1–3 s p95 — die 64-Clauses-`rank_feature`-bool-should ist auf diesem Index teuer. Erwartetes Einsatzprofil bleibt rare Tokens (Normnummern/Akronyme über Sprachgrenzen) — dafür gibt es in der Suite bisher zu wenige solcher Queries (n18–n21 messen es nicht isoliert).
4. **Graph-Arm (R6): leicht schädlich + teuer.** MRR −0.001, R@10 −0.020, ~+2 s p95 (die GraphCandidates-SQL über 55k Mentions ist ungetunt). Default-OFF bestätigt.
5. **Latenz-Budget (top_n=10):** rerank kostet lokal ~4–7 s p50 (30 Kandidaten × ~130 ms MPS fp32). Für das 2-s-Budget: Remote-Runner (CUDA, R4-gemessen 0.95 s p95 bei top_n=5), Overfetch 2× (~20 Kandidaten ≈ −33 %), oder top_n=5. Der R3-SLA gilt für top_n=5.
...
| Sparse | **aus** (`AXIOM_SEARCH_SPARSE_ARM`, umgestellt mit diesem Benchmark) | kein Qualitäts­gewinn, ~3 s p95; Hebel für rare-Token-Profile nach Tuning (sparseTopK, Shards) |
