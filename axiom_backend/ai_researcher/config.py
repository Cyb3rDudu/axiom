...
try:
    ...
    print(
        f"⚠️ Invalid timezone '{timezone_name}' found in TZ environment variable. Defaulting to UTC."
    )
...
try:
    from .dynamic_config import (
        ...
        get_mid_llm_provider,
        get_intelligent_llm_provider,
        get_verifier_llm_provider,
        ...
        get_max_research_cycles_per_section,
        get_max_total_iterations,
        get_max_total_depth,
    ...
    def get_setting_with_fallback(
        setting_name: str, default_value: Any = None, setting_type: type = str
    ) -> Any:
...
FORCE_CPU_MODE = (
    os.getenv("FORCE_CPU_MODE", "false").lower() == "true"
)  # Force CPU mode for all operations
PREFERRED_DEVICE_TYPE = os.getenv(
...
if not is_running_in_docker():
    ...
    if FORCE_CPU_MODE:
        ...
        print(
            f"Running in Docker. CUDA_VISIBLE_DEVICES is {docker_cuda_devices} (managed by Docker)"
        )
...
)  # Often 'none' or not required for local
...
DEEPSEEK_INTELLIGENT_MODEL = os.getenv(
...
AGENT_ROLE_MAX_TOKENS = {
    ...
    "note_assignment": int(
        os.getenv("NOTE_ASSIGNMENT_MAX_TOKENS", 8192)
    ),  # Added role with higher limit
    "verifier": int(
...
def get_model_name(model_type: str) -> str:
    ...
    if DYNAMIC_CONFIG_AVAILABLE:
        ...
        if (
            model_type == "fast" or model_type == "light"
        ):  # Support both new and old names
            provider = FAST_LLM_PROVIDER
            model_name = (
                LOCAL_LLM_FAST_MODEL if provider == "local" else OPENROUTER_FAST_MODEL
            )
        elif (
            model_type == "mid" or model_type == "heavy"
        ):  # Support both new and old names
            provider = MID_LLM_PROVIDER
            model_name = (
                LOCAL_LLM_MID_MODEL if provider == "local" else OPENROUTER_MID_MODEL
            )
        elif (
            model_type == "intelligent" or model_type == "beast"
        ):  # Support both new and old names
            provider = INTELLIGENT_LLM_PROVIDER
            model_name = (
                LOCAL_LLM_INTELLIGENT_MODEL
                if provider == "local"
                else OPENROUTER_INTELLIGENT_MODEL
            )
            ...
            model_name = (
                ...
                if provider == "local"
                else OPENROUTER_VERIFIER_MODEL
            )
        else:
            raise ValueError(
                f"Unknown model type '{model_type}' requested. Please configure user AI settings."
            )

        if not model_name:
            raise ValueError(
                f"No {model_type} model configured for provider '{provider}'. Please set user AI settings or environment variables."
            )
...
LLM_REQUEST_TIMEOUT = float(
    os.getenv("LLM_REQUEST_TIMEOUT", 600.0)
...
WEB_SEARCH_COST_PER_CALL = float(
    os.getenv("WEB_SEARCH_COST_PER_CALL", 0.005)
)  # Default $0.005 per search

# --- Web Fetcher Cache Configuration ---
WEB_CACHE_EXPIRATION_DAYS = int(
    os.getenv("WEB_CACHE_EXPIRATION_DAYS", 2)
)  # Days to keep cached web pages
...
DATABASE_URL = os.getenv(
    "DATABASE_URL", "sqlite:///./ai_researcher/data/vector_store/chroma.db"
)  # Example path
...
MAX_TOTAL_ITERATIONS = (
    get_max_total_iterations()
    if DYNAMIC_CONFIG_AVAILABLE
    else int(os.getenv("MAX_TOTAL_ITERATIONS", 40))
)  # Overall limit for research/reflection iterations
...
MAX_RESEARCH_CYCLES_PER_SECTION = (
    get_max_research_cycles_per_section()
    if DYNAMIC_CONFIG_AVAILABLE
    else int(os.getenv("MAX_RESEARCH_CYCLES_PER_SECTION", 2))
)  # Default is 2
MAX_DEPTH_PASS1 = int(
    os.getenv("MAX_DEPTH_PASS1", 1)
)  # Max outline depth processed in Pass 1 (Less relevant now?)
MAX_TOTAL_DEPTH = (
    get_max_total_depth()
    if DYNAMIC_CONFIG_AVAILABLE
    else int(os.getenv("MAX_TOTAL_DEPTH", 2))
)  # Max outline depth allowed after Inter-Pass revision

# --- Advanced Workflow Configuration ---
INITIAL_RESEARCH_MAX_DEPTH = (
    get_initial_research_max_depth()
)  # 3 Max depth for initial question tree
INITIAL_RESEARCH_MAX_QUESTIONS = (
    get_initial_research_max_questions()
)  # 50 Max total questions in initial phase
STRUCTURED_RESEARCH_ROUNDS = (
    get_structured_research_rounds()
)  # def 2: Number of structured research rounds (Set to 1 for single pass)
WRITING_PASSES = (
    get_writing_passes()
)  # Default 3: Number of writing passes (initial + revisions)
# Max characters for notes context passed to PlanningAgent in one go
MAX_PLANNING_CONTEXT_CHARS = (
    get_max_planning_context_chars()
    if DYNAMIC_CONFIG_AVAILABLE
    else int(os.getenv("MAX_PLANNING_CONTEXT_CHARS", 250000))
)
# Max characters for previewing previously written content passed to WritingAgent
WRITING_PREVIOUS_CONTENT_PREVIEW_CHARS = (
    get_writing_previous_content_preview_chars()
    if DYNAMIC_CONFIG_AVAILABLE
    else int(os.getenv("WRITING_PREVIOUS_CONTENT_PREVIEW_CHARS", 30000))
)  # Max characters for previewing previously written content (each section)
# Number of recent thoughts to provide as context to agents
THOUGHT_PAD_CONTEXT_LIMIT = (
    get_thought_pad_context_limit()
)  # Default to 5 recent thoughts
...
RESEARCH_NOTE_CONTENT_LIMIT = (
    get_research_note_content_limit()
    if DYNAMIC_CONFIG_AVAILABLE
    else int(os.getenv("RESEARCH_NOTE_CONTENT_LIMIT", 32000))
)  # Default 32000: Max characters of source content to feed into note generation prompt

# --- Embedding Configuration ---
EMBEDDING_BATCH_SIZE = int(
    os.getenv("EMBEDDING_BATCH_SIZE", 8)
)  # Default 8: Batch size for embedding operations (reduced for memory safety)
EMBEDDING_MAX_CONCURRENT_QUERIES = int(
    os.getenv("EMBEDDING_MAX_CONCURRENT_QUERIES", 3)
)  # Default 3: Max concurrent embedding queries
EMBEDDING_MEMORY_MANAGEMENT = (
    os.getenv("EMBEDDING_MEMORY_MANAGEMENT", "True").lower() == "true"
...
INITIAL_EXPLORATION_DOC_RESULTS = (
    get_initial_exploration_doc_results()
)  # default 10: Number of docs for initial exploration
INITIAL_EXPLORATION_WEB_RESULTS = (
    get_initial_exploration_web_results()
)  # default 2: Number of web results for initial exploration
INITIAL_EXPLORATION_USE_RERANKER = (
    os.getenv("INITIAL_EXPLORATION_USE_RERANKER", "True").lower() == "true"
)  # Whether to rerank initial exploration results

# --- Main Research Phase ---
MAIN_RESEARCH_DOC_RESULTS = (
    get_main_research_doc_results()
)  # default 10: Number of docs for main research cycles
MAIN_RESEARCH_WEB_RESULTS = (
    get_main_research_web_results()
)  # default 5: Number of web results for main research cycles

# Note Assignment Configuration
MIN_NOTES_PER_SECTION_ASSIGNMENT = (
    get_min_notes_per_section_assignment()
    if DYNAMIC_CONFIG_AVAILABLE
    else int(os.getenv("MIN_NOTES_PER_SECTION_ASSIGNMENT", 5))
)  # Minimum notes per section
MAX_NOTES_PER_SECTION_ASSIGNMENT = (
    get_max_notes_per_section_assignment()
    if DYNAMIC_CONFIG_AVAILABLE
    else int(os.getenv("MAX_NOTES_PER_SECTION_ASSIGNMENT", 40))
)  # Maximum notes per section
