...
class BaseAgent(ABC):
    ...
    def __init__(
        ...
        tool_registry: Optional[
            ToolRegistry
        ] = None,  # Tools might be optional for some agents
        system_prompt: Optional[str] = None,
        model_name: Optional[
            str
        ] = None,  # Optional: Specific model override for this agent
        ...
        if system_prompt is None:
            try:
                ...
                if not isinstance(e, RuntimeError):
                    print(
                        f"Warning: Could not load prompt for {agent_name} in {language_code}, using default: {e}"
                    )

        self.system_prompt = system_prompt
        self.model_name = (
            model_name  # If None, ModelDispatcher will use default for the mode
        )

        print(
            f"Initialized {self.agent_name} (Model: {self.model_name or 'Default'}, Lang: {self.language_code})"
        )
    ...
    def reload_prompts(self, language_code: str):
        ...
        print(
            f"Reloading prompts for {self.agent_name}: {self.language_code} → {language_code}"
        )
        self.language_code = language_code
        ...
        try:
            ...
            print(
                f"Warning: Could not reload prompt for {self.agent_name} in {language_code}, using default: {e}"
            )
    ...
    async def _call_llm(  # <-- Make async
        ...
        tool_choice: Optional[
            Any
        ] = None,  # e.g., "auto", "required", {"type": "function", "function": {"name": "my_function"}}
        response_format: Optional[
            Dict[str, str]
        ] = None,  # e.g., {"type": "json_schema"}
        ...
        **kwargs: Any,  # Accept arbitrary keyword arguments
        ...
        try:
            ...
            model_to_use = kwargs.pop(
                "model", self.model_name
            )  # Get 'model' from kwargs, default to self.model_name, and remove it from kwargs
            ...
            (
                response,
                model_call_details,
            ) = await self.model_dispatcher.dispatch(  # <-- Use await
                messages=messages,
            ...
            if should_log:
                ...
                if (
                    ...
                    and self.controller
                    and hasattr(self.controller, "context_manager")
                ):
                ...
                if context_manager and mission_id and log_queue and update_callback:
                    ...
                    completion_tokens = (
                        model_call_details.get("completion_tokens", 0)
                        if model_call_details
                        else 0
                    )
                    prompt_tokens = (
                        model_call_details.get("prompt_tokens", 0)
                        if model_call_details
                        else 0
                    )
                    ...
                    if agent_mode == "research" and completion_tokens <= 20:
                        ...
                        if "content reviewed, but not relevant" in prompt_lower:
                            ...
                            for pattern in patterns:
                                match = re.search(
                                    pattern, user_prompt[:2000], re.IGNORECASE
                                )
                                if match:
            ...
            if should_retry_with_json_object(e) or should_retry_without_response_format(
                e
            ):
                raise e
            print(
                f"{self.agent_name} Error: Failed to get response from LLM via ModelDispatcher: {e}"
            )
            if log_llm_call:
                context_manager = None
                if (
    ...
    async def _execute_tool(
        ...
        if not registry_to_use:
            print(
                f"{self.agent_name} Error: ToolRegistry not available (neither self.tool_registry nor override provided)."
            )
            return {"error": "Tool execution capability not available."}

        # --- REVISED DEBUG LOGGING ---
        log_message = (
            f"DEBUG ({self.agent_name}): _execute_tool received args for '{tool_name}'."
        )
        ...
        if (
            "filepath" in tool_arguments
            and "allowed_base_path" in tool_arguments
            and tool_name == "read_full_document"
        ):
            received_filepath_str = tool_arguments["filepath"]  # Get actual value
            allowed_base_path_str = tool_arguments[
                "allowed_base_path"
            ]  # Get actual value
            try:
                ...
                if allowed_base_path_str is None or allowed_base_path_str == "None":
                    print(
                        f"DEBUG ({self.agent_name}): Skipping path check because allowed_base_path is None or 'None'"
                    )
                else:
                    ...
                    if not received_filepath_str.startswith(str(allowed_base)):
                        print(
                            f"CRITICAL WARNING ({self.agent_name}): _execute_tool received MISMATCHED filepath '{received_filepath_str}' which does not start with allowed base '{allowed_base}'!"
                        )
            except Exception as path_ex:
                print(
                    f"DEBUG ({self.agent_name}): Path check exception in _execute_tool: {path_ex}"
                )
        # --- End Path Mismatch Check ---

        try:
            # --- ADD PRE-REGISTRY CALL DEBUG ---
            final_filepath = tool_arguments.get(
                "filepath", "FILEPATH_KEY_MISSING_PRE_REGISTRY"
            )
            print(
                f"DEBUG ({self.agent_name}): PRE-REGISTRY CALL for '{tool_name}'. Filepath value: '{final_filepath}'"
            )
            ...
            if tool_name == "fetch_web_page_content" and "url" in tool_arguments:
                ...
                from ai_researcher.agentic_layer.tools.arxiv_fetcher_tool import (
                    ArXivFetcherTool,
                )

                is_arxiv, arxiv_id = ArXivFetcherTool.is_arxiv_url(url)
                print(
                    f"DEBUG: Checking URL '{url}' - is_arxiv={is_arxiv}, arxiv_id={arxiv_id}"
                )
                if is_arxiv:
                    print(
                        f"{self.agent_name}: Detected arXiv URL, using specialized arXiv fetcher for: {url} (ID: {arxiv_id})"
                    )
                    ...
                    if (
                        ...
                        if context_manager:
                            await context_manager.log_execution_step(
                                ...
                                input_summary=url,
                                output_summary="arXiv paper fetched successfully"
            ...
            if not tool_def:
                print(
                    f"{self.agent_name} Error: Tool '{tool_name}' not found in the registry."
                )
            ...
            final_args = {
                ...
                if k in tool_params or k == "kwargs"
            }

            # Check for and add extra context arguments if the tool accepts them and the agent has them
            context_args_map = {
                "mission_id": getattr(self, "mission_id", None),
                "agent_controller": getattr(
                    self, "controller", None
                ),  # Assuming controller is stored as self.controller
            ...
            for param_name, agent_attr_value in context_args_map.items():
                if param_name in tool_params and agent_attr_value is not None:
                    ...
                    if param_name not in final_args:  # Avoid overwriting args from LLM
                        ...
                        print(
                            f"DEBUG ({self.agent_name}): Adding context arg '{param_name}' to tool '{tool_name}' call."
                        )
                    else:
                        print(
                            f"DEBUG ({self.agent_name}): Context arg '{param_name}' already present in LLM args for tool '{tool_name}'. Skipping."
                        )

            # Await the registry's async execute method using the determined registry and final arguments
            result = await registry_to_use.execute_tool(
                tool_name, final_args
            )  # Pass the augmented args
            ...
            if isinstance(result, dict) and "error" in result:
                ...
                result is None
            ):  # Handle case where execute_tool itself might fail before tool impl
                log_status = "failure"
                ...
                try:
                    if isinstance(result, list):
                        ...
                        result_summary = (
                            f"Returned dict with {len(result['results'])} results."
                        )
                    elif isinstance(result, str):
                        result_summary = f"Returned string (length: {len(result)})."
                    else:
                        result_summary = (
                            f"Returned result of type {type(result).__name__}."
                        )
                except Exception:
            ...
            if (
                ...
                if tool_name == "read_full_document":
                    if "original_filename" in tool_arguments:
                        action_text = (
                            f"Read Document: {tool_arguments['original_filename']}"
                        )
                ...
                if not context_manager:
                    missing.append("context_manager")
                # Check for mission_id attribute existence AND if it has a truthy value
                if not hasattr(self, "mission_id") or not self.mission_id:
                    missing.append("mission_id")
                if not log_queue:
                    missing.append("log_queue")
                if not effective_update_callback:
                    missing.append("update_callback")
                print(
                    f"WARNING ({self.agent_name}): Could not log tool execution for '{tool_name}'. Missing: {', '.join(missing)}"
                )
                # --- END MODIFICATION ---
