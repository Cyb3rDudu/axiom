"""
Enhanced Collaborative Writing Agent for AXIOM - Agentic Workflow
"""
...
from ..utils.json_format_helper import (
...
"""
    ...
    async def run(
        ...
        **kwargs,
        ...
        logger.info(
            f"{self.agent_name}: Starting agentic workflow for draft {draft_id}"
        )
        ...
        try:
            ...
            is_confirmation = context_override and context_override.get(
            ...
            if is_confirmation:
                ...
                return await self._handle_confirmation(
                    context_override, draft_id, user_id, log_queue, update_callback
                )
            ...
            logger.error(
                f"Error in EnhancedCollaborativeWritingAgent.run: {e}", exc_info=True
            )
    ...
    def _create_planning_prompt(
        ...
        sections_info = [
            ...
            for s in document_outline.get("sections", [])
        ]
                    ...
                    schema_hints = """
{
  "plan": [
    {"tool_name": "string", "arguments": {"key": "value"}}
                    ...
                    enhanced = enhance_messages_for_json_object(
                        [{"role": "user", "content": planning_prompt}], schema_hints
                    )
                    current_prompt = enhanced[0]["content"]
                ...
                if llm_response and llm_response.choices:
                    plan_json = parse_llm_json_response(
                ...
                if (
                    ...
                    logger.info("Falling back to prompt-only mode")
                    format_mode = "none"
                    retry_count = 0
                    continue
        ...
        if not plan:
            return (
                {
                    "message": "I'm not sure how to proceed. Could you please clarify your request?"
                },
                None,
                "No plan generated.",
            )
        ...
        if update_callback:
            await update_callback(
                {"type": "agent_status", "status": f"Executing: {tool_name}..."}
            )

        result = await self._execute_tool(
            tool_name, arguments, log_queue=log_queue, update_callback=update_callback
        )
    ...
    async def _handle_confirmation(
        self, context_override, draft_id, user_id, log_queue, update_callback
    ):
        ...
        if update_callback:
            await update_callback(
                {
                    "type": "agent_status",
                    "status": f"Confirmed. Executing: {execution_tool_name}...",
                }
            )
