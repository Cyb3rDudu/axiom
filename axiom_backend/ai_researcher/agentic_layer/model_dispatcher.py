...
from decimal import (
    Decimal,
    InvalidOperation,
)  # <-- Import Decimal for accurate cost calculation
import json
import os
...
class ModelDispatcher:
    ...
    def __init__(
        ...
        logger.info(
            f"Dispatcher initializing for user with providers: {providers_in_use}"
        )
        ...
        for provider_name in providers_in_use:
            ...
            logger.info(
                f"[DEBUG] User config for {provider_name}: {user_provider_config}"
            )
            ...
            logger.info(
                f"[DEBUG] Provider {provider_name}: API key {api_key_status} ({api_key_preview}), Base URL: {base_url}"
            )
            ...
            if not api_key:
                if provider_name == "custom":
                    ...
                    logger.info(
                        f"Using dummy API key for custom provider without authentication"
                    )
                else:
                    logger.debug(
                        f"API key for provider '{provider_name}' not provided. Client will be initialized when user settings are available."
                    )
            ...
            if not base_url:
                logger.error(
                    f"Base URL for provider '{provider_name}' is not configured. Client cannot be initialized."
                )
                self.clients[provider_name] = None
                continue
            ...
            try:
                ...
                logger.info(
                    f"AsyncOpenAI client initialized successfully for provider: {provider_name} at {base_url}"
                )
            except Exception as e:
                logger.error(
                    f"Error initializing AsyncOpenAI client for provider {provider_name}: {e}",
                    exc_info=True,
                )
                self.clients[provider_name] = None
    ...
    def _get_providers_from_settings(self) -> set:
        ...
        if self.user_settings and "ai_endpoints" in self.user_settings:
            ...
            advanced_models = self.user_settings["ai_endpoints"].get(
                "advanced_models", {}
            )
        ...
        for provider_name, client in self.clients.items():
            if client is not None:
                try:
                    ...
                    logger.debug(
                        f"Closed AsyncOpenAI client for provider: {provider_name}"
                    )
                except Exception as e:
                    logger.warning(
                        f"Error closing AsyncOpenAI client for provider {provider_name}: {e}"
                    )
        self.clients.clear()

    def _get_or_create_client(
        self, provider_name: str, current_user_settings: Optional[Dict[str, Any]]
        ...
        logger.info(
            f"[DEBUG] _get_or_create_client for {provider_name}: API key {'present' if api_key else 'missing'}, Base URL: {base_url}"
        )

        if not api_key or not base_url:
            logger.error(
                f"Missing credentials for provider '{provider_name}': API key {'missing' if not api_key else 'present'}, Base URL {'missing' if not base_url else 'present'}"
            )
            return None
        ...
        if existing_client:
            ...
            logger.info(
                f"[DEBUG] Creating fresh client for {provider_name} with updated credentials"
            )

        try:
            client = AsyncOpenAI(
                base_url=base_url, api_key=api_key, timeout=config.LLM_REQUEST_TIMEOUT
            )
            # Update the cached client
            self.clients[provider_name] = client
            logger.info(
                f"[DEBUG] Fresh AsyncOpenAI client created successfully for provider: {provider_name} at {base_url}"
            )
            return client
        except Exception as e:
            logger.error(
                f"Error creating fresh AsyncOpenAI client for provider {provider_name}: {e}",
                exc_info=True,
            )
            return None
    ...
    def _select_model_and_client(
        ...
        logger.info(
            f"[_select_model_and_client] Received: requested_model='{requested_model}', agent_mode='{agent_mode}'"
        )
        # <<< END CHANGE >>>
        ...
        )  # Use default if no mode specified
        ...
        logger.info(
            f"[DEBUG] Fresh user settings retrieved: {bool(current_user_settings)}"
        )
        ...
        if current_user_settings and "ai_endpoints" in current_user_settings:
            ...
            logger.info(
                f"[DEBUG] Available advanced_models: {list(advanced_models.keys())}"
            )
            ...
            if model_type in advanced_models:
                ...
                if not provider_name or not model_name:
                    logger.warning(
                        f"[DEBUG] Incomplete config for '{model_type}' in advanced_models: {model_config}. Attempting dynamic_config.get_model_name as fallback."
                    )
                    try:
                        ...
                        logger.info(
                            f"[DEBUG] Fallback recovered values: provider='{provider_name}', model='{model_name}'"
                        )
                    except Exception as e:
                        logger.error(
                            f"[DEBUG] dynamic_config.get_model_name failed for '{model_type}': {e}"
                        )

                # Override with requested_model if provided
                if requested_model:
                    logger.warning(
                        f"Specific model '{requested_model}' requested, overriding user setting '{model_name}'."
                    )
                    model_name = requested_model
            else:
                logger.warning(
                    f"Model type '{model_type}' not found in user's advanced_models. Attempting dynamic_config.get_model_name as ultimate fallback."
                )
                try:
                    model_name = get_model_name(model_type)
                    if model_name.startswith("local/"):
        ...
        if not provider_name or not model_name:
            logger.error(
                f"Model configuration missing or incomplete for model type '{model_type}'."
            )
            raise ValueError(
                f"No valid model configured for '{model_type}'. User settings exist but values may be incomplete."
            )
        ...
        if not client:
            logger.error(
                f"Async Client for provider '{provider_name}' is not available or could not be initialized."
            )
            return None, None, None
    ...
    def _load_model_pricing(self):
        ...
        try:
            ...
            per_tokens = pricing_data.get(
                "per_tokens", 1000000
            )  # Default to per million

            for model_id, prices in models.items():
                # Convert from per-million to per-token pricing
                input_price = Decimal(str(prices.get("input", 0))) / Decimal(
                    str(per_tokens)
                )
                output_price = Decimal(str(prices.get("output", 0))) / Decimal(
                    str(per_tokens)
                )
    ...
    async def _fetch_and_cache_pricing(self):
        ...
        if not api_key:
            logger.warning(
                "Cannot fetch OpenRouter pricing: OPENROUTER_API_KEY not found in user settings or global config."
            )
            return
        ...
        async with httpx.AsyncClient(timeout=config.LLM_REQUEST_TIMEOUT) as client:
            try:
                ...
                if (
                    not isinstance(models_data, dict)
                    or "data" not in models_data
                    or not isinstance(models_data["data"], list)
                ):
                    logger.error(
                        f"Unexpected format received from OpenRouter /models endpoint: {models_data}"
                    )
                    return
                ...
                for model_info in models_data["data"]:
                    ...
                    if model_id and isinstance(pricing, dict):
                        try:
                            ...
                            logger.warning(
                                f"Could not parse pricing for model '{model_id}': {pricing}. Error: {e}. Setting costs to 0."
                            )
                            new_cache[model_id] = {
                                "prompt": Decimal("0"),
                                "completion": Decimal("0"),
                            }  # Store default on error

                # Update the cache directly (removed lock)
                self.model_pricing_cache = new_cache
                logger.info(
                    f"Successfully fetched and cached pricing for {len(self.model_pricing_cache)} models from OpenRouter."
                )

            except httpx.RequestError as e:
                logger.error(
                    f"Error fetching OpenRouter model pricing (Request Error): {e}",
                    exc_info=True,
                )
            except httpx.HTTPStatusError as e:
                logger.error(
                    f"Error fetching OpenRouter model pricing (HTTP Status {e.response.status_code}): {e.response.text}",
                    exc_info=True,
                )
            except Exception as e:  # Catch broader exceptions like JSONDecodeError
                logger.error(
                    f"Unexpected error fetching or processing OpenRouter model pricing: {e}",
                    exc_info=True,
                )
    ...
    async def dispatch(  # <-- Make method async
        ...
        update_callback: Optional[
            Any
        ] = None,  # <-- Add update_callback parameter (currently unused but added for consistency)
        mission_id: Optional[
            str
        ] = None,  # <-- Add mission_id parameter for status checking
        **kwargs: Any,  # Accept additional keyword arguments (like agent_logged)
    ) -> Tuple[
        Optional[Any], Optional[Dict[str, Any]]
    ]:  # Return type: (ChatCompletion, model_details_dict) or (None, None)
        """
        ...
        if self.context_manager and mission_id:
            try:
                ...
                if mission_context and mission_context.status in ["stopped", "paused"]:
                    logger.info(
                        f"Mission {mission_id} is {mission_context.status}. Cancelling LLM call to prevent new tasks."
                    )
                    return None, {
                        "cancelled": True,
                        "reason": f"Mission {mission_context.status}",
                    }
            except Exception as status_check_error:
                logger.warning(
                    f"Error checking mission status for {mission_id}: {status_check_error}. Proceeding with LLM call."
                )

        # Type hint for the async client
        client: Optional[AsyncOpenAI]
        client, selected_model_name, provider_name = self._select_model_and_client(
            requested_model=model, agent_mode=agent_mode
        )

        if not client or not selected_model_name or not provider_name:
            logger.error(
                "Could not select a valid Async LLM client, model name, or provider. Cannot dispatch request."
            )
            return None, None

        # Determine max_tokens AND temperature based on agent_mode using the config
        effective_agent_mode = agent_mode or "default"
        max_tokens_for_call = config.AGENT_ROLE_MAX_TOKENS.get(
            effective_agent_mode, config.AGENT_ROLE_MAX_TOKENS["default"]
        )
        # --- NEW: Get temperature from config ---
        temperature_for_call = config.AGENT_ROLE_TEMPERATURE.get(
        ...
        is_openai_or_azure = provider_name == "openai" or (
            provider_name == "custom" and is_azure
        )

        if is_azure and is_gpt5_model:
            logger.info(
                f"Detected Azure OpenAI with GPT-5 model: {selected_model_name}, using max_completion_tokens"
            )
        ...
        try:
            logger.info(
                "[DEBUG] Dispatch request parameters: "
            ...
            logger.error(
                f"[DEBUG] Failed to log dispatch request params: {dbg_e}", exc_info=True
            )
        ...
        for attempt in range(self.max_retries):
            ...
            try:
                ...
                logger.info(
                    f"Async LLM call successful using model '{selected_model_name}' in {duration:.2f}s (Attempt {attempt + 1}/{self.max_retries})"
                )
                ...
                if response and response.usage:
                    ...
                    if provider_name == "openrouter":
                        ...
                        )  # Ensure pricing is loaded (lazy load)
                        model_pricing = self.model_pricing_cache.get(
                            selected_model_name
                        )
                        if model_pricing:
                            ...
                            logger.warning(
                                f"Pricing information not found in cache for model: {selected_model_name}. Cost cannot be calculated."
                            )
                            model_call_details["cost"] = (
                                0.0  # Set cost to 0 if pricing not found
                            )
                    ...
                    logger.info(
                        f"Calculated cost for {selected_model_name}: ${final_cost_float:.6f}"
                    )
                    ...
                    logger.warning(
                        "Usage information not found in the response object. Cost cannot be calculated."
                    )
                ...
                if (
                ...
                ):  # Check message exists, content check below
                    ...
                    has_valid_empty_response = is_router_mode and isinstance(
                    ...
                    if has_content or has_tool_calls or has_valid_empty_response:
                        ...
                        if has_valid_empty_response and not has_content:
                            logger.info(
                                f"Router/query_strategy mode returned empty response (likely thinking model). Accepting as valid."
                            )
                        ...
                        if log_queue:
                            try:
                                ...
                                model_details_message = {
                                    "type": "model_call_details",
                                    "data": model_call_details.copy(),
                                }
                                ...
                                if update_callback:
                                    try:
                                        ...
                                        update_callback(
                                            log_queue, model_details_message
                                        )
                                        logger.debug(
                                            f"Called update_callback with model_call_details for model {selected_model_name}"
                                        )
                                        cost_saved = True
                                    except Exception as cb_err:
                                        logger.error(
                                            f"Failed to call update_callback with model details: {cb_err}"
                                        )
                        ...
                        if self.context_manager and mission_id and model_call_details:
                            ...
                            model_call_details_with_mode["agent_mode"] = (
                                effective_agent_mode
                            )
                            ...
                            logger.debug(
                                f"ModelDispatcher calling update_mission_stats for {effective_agent_mode} with cost ${model_call_details.get('cost', 0):.6f} (agent_logged={agent_logged})"
                            )

                            # Schedule the async update_mission_stats call
                            asyncio.create_task(
                                self.context_manager.update_mission_stats(
                                    mission_id,
                        ...
                        logger.warning(
                            f"LLM response content is empty or not a string, and no tool calls present (Attempt {attempt + 1}). Type: {type(problematic_content)}, Content: {problematic_content}"
                        )
                    ...
                    try:
                        response_details = (
                            response.model_dump_json(indent=2) if response else "None"
                        )  # Use Pydantic's dump for clarity
                    except Exception:
                        response_details = str(
                            response
                        )  # Fallback to string representation
                    logger.warning(
                        f"LLM response structure invalid (missing choices/message) (Attempt {attempt + 1}). Response object details:\n{response_details}"
                    )
            ...
            # Catch specific async-related errors
                ...
                if provider_name == "openrouter":
                    ...
                    estimated_cost = (
                        estimated_prompt_tokens * 0.000001
                    )  # Rough estimate
                    logger.info(
                ...
                if attempt < self.max_retries - 1:
                    ...
                    logger.warning(
                        f"Rate limit error (Attempt {attempt + 1}/{self.max_retries}): {e}. Retrying in {sleep_duration:.2f}s..."
                    )
                    await asyncio.sleep(sleep_duration)
                else:
                    logger.error(
                        f"LLM call failed after {self.max_retries} attempts due to RateLimitError: {e}"
                    )
                ...
                if attempt < self.max_retries - 1:
                    ...
                    sleep_duration = backoff_delay + jitter
                    logger.warning(
                        f"API connection error (Attempt {attempt + 1}/{self.max_retries}): {e}. Retrying in {sleep_duration:.2f}s..."
                    )
                    await asyncio.sleep(sleep_duration)
                else:
                    logger.error(
                        f"LLM call failed after {self.max_retries} attempts due to APIConnectionError: {e}"
                    )
                    return None, None  # Failed after all retries
                ...
                if e.status_code == 400 and is_openai_or_azure:
                    ...
                    if (
                        ...
                        logger.info(
                            f"Detected GPT-5 parameter error on {provider_type}, retrying with max_completion_tokens..."
                        )
                        ...
                        if (
                            ...
                            and request_params["temperature"] != 1
                        ):
                        ...
                        try:
                            ...
                            logger.info(
                                f"GPT-5 fallback successful with max_completion_tokens"
                            )
                            ...
                            if (
                                ...
                                if response.usage:
                                    model_call_details["prompt_tokens"] = (
                                        response.usage.prompt_tokens or 0
                                    )
                                    model_call_details["completion_tokens"] = (
                                        response.usage.completion_tokens or 0
                                    )
                                    model_call_details["total_tokens"] = (
                    ...
                    # Check for temperature not supported error
                        ...
                        logger.info(
                            f"Detected GPT-5 temperature error, retrying without custom temperature..."
                        )
                ...
                if e.status_code == 400 and should_retry_with_json_object(e):
                    ...
                    logger.warning(
                        f"API status error appears to be schema-related (Status=400): {str(e)[:200]}... Re-raising for potential fallback."
                    )
                    raise e
                else:
                    logger.error(
                        f"API status error (Attempt {attempt + 1}/{self.max_retries}): Status={e.status_code}, Response={e.response}. No retry for status errors.",
                        exc_info=True,
                    )
                ...
                if attempt < self.max_retries - 1:
                    ...
                    sleep_duration = backoff_delay + jitter
                    logger.error(
                        f"Unexpected error during LLM call (Attempt {attempt + 1}/{self.max_retries}): {e}. Retrying in {sleep_duration:.2f}s...",
                        exc_info=True,
                    )
                    await asyncio.sleep(sleep_duration)
                else:
                    logger.error(
                        f"LLM call failed after {self.max_retries} attempts due to an unexpected error: {e}",
                        exc_info=True,
                    )
                    return None, None  # Failed after all retries
            ...
            if attempt < self.max_retries - 1:
                ...
                is_invalid_response = "response" in locals() and (
                    ...
                    or (
                        ...
                        and not bool(response.choices[0].message.tool_calls)
                    )
                )
                if is_invalid_response:
                    ...
                    sleep_duration = backoff_delay + jitter
                    logger.warning(
                        f"Invalid or empty response detected (Attempt {attempt + 1}/{self.max_retries}). Retrying in {sleep_duration:.2f}s..."
                    )
                    await asyncio.sleep(sleep_duration)
            ...
            if attempt == self.max_retries - 1:
                ...
                if is_invalid_response_final:
                    logger.error(
                        f"LLM call failed after {self.max_retries} attempts due to invalid or empty response on the final attempt."
                    )
        ...
        logger.error(
            "LLM call failed after all retries (reached end of dispatch function)."
        )
        return None, None
    ...
    async def dispatch_stream(
        ...
        if self.context_manager and mission_id:
            try:
                ...
                if mission_context and mission_context.status in ["stopped", "paused"]:
                    logger.info(
                        f"Mission {mission_id} is {mission_context.status}. Cancelling streaming LLM call."
                    )
                    return
            except Exception as status_check_error:
                logger.warning(
                    f"Error checking mission status for {mission_id}: {status_check_error}. Proceeding with streaming LLM call."
                )
        ...
        if not client or not selected_model_name or not provider_name:
            logger.error(
                "Could not select a valid Async LLM client, model name, or provider. Cannot dispatch streaming request."
            )
            return
        ...
        try:
            ...
            if self.semaphore and global_semaphore:
                ...
                logger.info(
                    f"Started streaming LLM call using model '{selected_model_name}' (both semaphores released)"
                )
            elif self.semaphore:
                async with self.semaphore:
                    stream = await client.chat.completions.create(**request_params)
                logger.info(
                    f"Started streaming LLM call using model '{selected_model_name}' (user semaphore released)"
                )
            elif global_semaphore:
                async with global_semaphore:
                    stream = await client.chat.completions.create(**request_params)
                logger.info(
                    f"Started streaming LLM call using model '{selected_model_name}' (global semaphore released)"
                )
            else:
                stream = await client.chat.completions.create(**request_params)
                logger.info(
                    f"Started streaming LLM call using model '{selected_model_name}')"
                )
            ...
            duration = end_time - start_time
            logger.info(
                f"Completed streaming LLM call using model '{selected_model_name}' in {duration:.2f}s"
            )
            ...
            logger.error(
                f"Unexpected error during streaming LLM call: {e}", exc_info=True
            )
            raise e
