...
# --- venv relocation fixup (one-time at install): shebangs + pyvenv.cfg -------
# The venv is built here at $STAGE/env; after extraction at another prefix the
# script bin/fix-env.sh rewrites the absolute paths.
cat > "$STAGE/env/bin/fix-env" <<'EOF'
#!/bin/sh
# One-time relocation fixup after extracting the fixer artifact (#204/#205).
# Rewrites venv shebangs and pyvenv.cfg to the current prefix.
set -eu
...
sed -i '' "s|home = $OLD|home = $HERE|" "$HERE/pyvenv.cfg" 2>/dev/null || true
echo "fix-env: relocated $OLD -> $HERE"
EOF
chmod +x "$STAGE/env/bin/fix-env"
printf '%s\n' "$STAGE/env" > "$STAGE/env/.build-prefix"
...
(cd "$STAGE/app" && "$BUILD_PREFIX/bin/python" -m pytest tests/test_import_guard.py -q 2>&1 | tail -1)
