...
export const DraftTab: React.FC<DraftTabProps> = ({ missionId }) => {
  ...
  const handleTransitionToWriting = async () => {
    ...
    try {
      // Always fetch fresh mission status — the store-cached Mission
      // object doesn't always hydrate the full metadata blob, so we
      // treat the /status endpoint as the authoritative source for
      // citation_profile_id, document_group_id, and the generated
      // group that was spun up during the run.
      const cachedMission = missions.find(m => m.id === missionId)
      let freshStatus: any = null
      try {
        const statusResponse = await apiClient.get(`/api/missions/${missionId}/status`)
        freshStatus = statusResponse.data
      } catch (err) {
        console.error('Could not fetch mission status:', err)
      }
      const missionMeta: any = (freshStatus?.metadata) || cachedMission?.metadata || {}
      const comprehensive: any = missionMeta.comprehensive_settings || {}

      // The auto-generated web-sources group (if any) lives on the
      // mission row as generated_document_group_id. The user's curated
      // research scope is the group they selected when starting the
      // mission — it sits in metadata.document_group_id /
      // metadata.document_group_ids. We union both so the new writing
      // session can search across web sources AND the book library.
      const autoGeneratedGroupId: string | null =
        freshStatus?.generated_document_group_id
        ?? cachedMission?.generated_document_group_id
        ?? null
      const researchGroupIds: string[] = (
        comprehensive.document_group_ids
        ?? missionMeta.document_group_ids
        ?? (
          missionMeta.document_group_id
            ? [missionMeta.document_group_id]
            : (comprehensive.document_group_id ? [comprehensive.document_group_id] : [])
        )
        ?? []
      ).filter(Boolean)

      const allGroupIds: string[] = []
      if (autoGeneratedGroupId) {
        allGroupIds.push(autoGeneratedGroupId)
      }
      ...
      // Primary group: prefer the auto-generated group (the one with the
      // web sources actually cited in the draft) when present, else the
      // first research group. This keeps the singular field meaningful
      // for backward-compat UI paths that show a single name.
      const documentGroupId: string | null = autoGeneratedGroupId
        ?? researchGroupIds[0]
        ?? null
